...
def drive_env_by_model_verbose(env_arg, num_of_frame_arg, model_arg, action_sapce_arg):
    observation, info = env_arg.reset()
    ...
    eta = 0.2
    for i in range(num_of_frame_arg):
        action = pick_action_verbose(model_arg, observation, action_sapce_arg)
        # print("=================")
        # print(type(action))
        # print(action)
        observation, reward, terminated, truncated, info = env_arg.step(action)
...
if __name__ == '__main__':
    ...
    for i in range(iter):
        ...
        if(i%3 == 0):
            x_input, y_output = drive_env_by_model(env_screen, 20, model_glb, action_space)
